...
VPNBOOK_FALLBACK_BUNDLES = ("US1", "US2", "CA1", "CA2", "UK1", "UK2", "DE1", "DE2", "FR1", "FR2")
...
def _get(url: str, timeout: float) -> bytes:
    req = urllib.request.Request(url, headers={
        "User-Agent": UA, "Accept": "text/html,text/plain,application/zip,*/*",
        "Accept-Encoding": "gzip, deflate", "Connection": "close", "Cache-Control": "no-cache",
    })
...
def _valid_servers(items) -> list[dict]:
    result = [x for x in (items or []) if isinstance(x, dict) and x.get("ip") and x.get("config_b64")]
...
def _api_servers() -> list[dict]:
    ...
    return _valid_servers(parse_servers(_get(API_URL.rstrip("/") + "/api/v1/public/servers?limit=120", 3.5)))
...
def _vpnbook_credentials(page: bytes) -> tuple[str, str]:
    ...
    if not password and "password" in low:
        tail = text[low.find("password"):]
        password = re.search(r">\s*([A-Za-z0-9]{6,16})\s*<", tail, re.I)
    # VPNBook currently publishes this password (updated Jul 18, 2026). Keep it as
    # a last-resort bootstrap so an HTML redesign cannot make the client show zero servers.
    return (user.group(1) if user else "vpnbook", password.group(1) if password else "ueedn87")
...
def _vpnbook_bundle_urls(page: bytes) -> list[tuple[str, str]]:
    ...
    if not found:
        found = [(n, VPNBOOK_BUNDLE_TEMPLATE.format(name=n)) for n in VPNBOOK_FALLBACK_BUNDLES]
    unique = {}
        ...
        try:
            ...
            with zipfile.ZipFile(io.BytesIO(raw)) as archive:
                for filename in archive.namelist():
                    ...
                    servers.append({
                        "id": f"vpnbook-{name}-{filename}", "ip": host, "hostname": host,
                        "country": name[:2].upper(), "city": name, "ping_ms": None,
                        "speed_mbps": 0.0, "uptime": 99.0, "score": score,
                        "config_b64": base64.b64encode(config.encode()).decode(), "source": "VPNBook",
                    })
            return servers
...
def _load_cache() -> tuple[list[dict], float]:
    try:
        ...
        return _valid_servers(data.get("servers", [])), float(data.get("time", 0))
    except Exception:
        return [], 0
...
def _refresh_live() -> list[dict]:
    ...
    try:
        ...
        while pending and time.monotonic() < deadline:
            done, pending = concurrent.futures.wait(pending, timeout=max(0.05, deadline - time.monotonic()), return_when=concurrent.futures.FIRST_COMPLETED)
            for future in done:
                try:
        ...
        if result:
            _save_cache(result); _log(f"live discovery: {len(result)} servers")
        else:
...
def _bootstrap_servers() -> list[dict]:
    ...
    for country, name, host in rows:
        ...
        result.append({
            ...
            "country": country, "city": name, "ping_ms": None, "speed_mbps": 0.0,
            "uptime": 99.0, "score": 20.0, "config_b64": base64.b64encode(b"bootstrap").decode(),
            "config_url": VPNBOOK_BUNDLE_TEMPLATE.format(name=bundle), "source": "VPNBook bootstrap",
        })
    return result
    ...
    if _refresh_lock.acquire(blocking=False):
        try:
            ...
            if cached and time.time() - stamp <= STALE_MAX_AGE:
                return cached
            bootstrap = _bootstrap_servers(); _save_cache(bootstrap); return bootstrap
        finally:
