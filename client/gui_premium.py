...
class App(EngineApp):
    """Premium consumer VPN UI: compact controls, dense server discovery, rich details."""
    ...
    def _configure_styles(self):
        ...
        s.configure("Treeview", background=PANEL, fieldbackground=PANEL, foreground=TEXT,
                    rowheight=42, borderwidth=0, relief="flat", font=(FONT, 9))
        s.configure("Treeview.Heading", background=PANEL_2, foreground=MUTED,
                    relief="flat", font=(FONT, 8, "bold"), padding=(10, 8))
        s.map("Treeview", background=[("selected", "#29223f")], foreground=[("selected", TEXT)])
        s.configure("TNotebook", background=BG, borderwidth=0)
        s.configure("TNotebook.Tab", background=PANEL_2, foreground=MUTED, padding=(12, 7), font=(FONT, 8, "bold"))
    ...
    def _btn(self, parent, text, command, kind="secondary", compact=False):
        colors = {
            ...
            "secondary": (PANEL_2, TEXT, "#252d3c"),
            "danger": (DANGER, "#080a0f", "#ff8296"),
        ...
        return tk.Button(parent, text=text, command=command, bg=bg, fg=fg,
                         ...
                         font=(FONT, 8 if compact else 9, "bold"), cursor="hand2",
                         padx=11 if compact else 16, pady=6 if compact else 8)
    ...
    def _build(self):
        ...
        tk.Label(self.sidebar, text="F", bg=ACCENT, fg="white", width=2, height=1,
                 font=(FONT, 18, "bold")).pack(pady=(22, 28))
        for icon, command in (("⌂", self._home), ("⌕", self._focus_search), ("⚡", self._fast), ("⚙", self._settings)):
            tk.Button(self.sidebar, text=icon, command=command, bg=SURFACE, fg=MUTED,
                      activebackground=PANEL_2, activeforeground=TEXT, relief="flat", bd=0,
                      font=("Segoe UI Symbol", 18), cursor="hand2").pack(fill="x", padx=10, pady=8)
        ...
        self.status_pill = tk.Label(header, text="● OFF", bg=PANEL_2, fg=MUTED, padx=12, pady=6,
                                    font=(FONT, 8, "bold"), highlightthickness=1, highlightbackground=BORDER)
        ...
        # Deliberately small: the main control is a button, not a giant screen-filling power circle.
        center = tk.Frame(hero, bg=SURFACE); center.grid(row=0, column=1, pady=15)
        self.power = tk.Button(center, text="CONNECT", command=self._toggle, bg=ACCENT, fg="white",
                               activebackground=ACCENT_HI, activeforeground="white", relief="flat", bd=0,
                               font=(FONT, 9, "bold"), width=11, height=1, padx=10, pady=8, cursor="hand2")
        self.power.pack(); self.timer = tk.Label(center, text="", bg=SURFACE, fg=MUTED, font=(FONT, 8)); self.timer.pack(pady=(6,0))
        ...
        head = tk.Frame(locations, bg=PANEL); head.grid(row=0, column=0, sticky="ew", padx=14, pady=(12,8))
        tk.Label(head, text="LOCATIONS", bg=PANEL, fg=TEXT, font=(FONT, 11, "bold")).pack(side="left")
        self.count = tk.Label(head, text="0 servers", bg=PANEL, fg=MUTED, font=(FONT, 8)); self.count.pack(side="right")
        search = tk.Frame(locations, bg=PANEL_2, highlightthickness=1, highlightbackground=BORDER); search.grid(row=1, column=0, sticky="ew", padx=14, pady=(0,8))
        tk.Label(search, text="⌕", bg=PANEL_2, fg=MUTED, font=(FONT, 13)).pack(side="left", padx=(9,3))
        self.search_var = tk.StringVar(); entry=tk.Entry(search,textvariable=self.search_var,bg=PANEL_2,fg=TEXT,insertbackground=TEXT,relief="flat",bd=0,font=(FONT,10)); entry.pack(fill="x",expand=True,padx=(0,9),pady=7); self.search_var.trace_add("write",lambda *_:self._render())
        self.entry = entry
        ...
        adv=self._card(side); adv.grid(row=1,column=0,sticky="nsew"); tk.Label(adv,text="ADVANCED",bg=PANEL,fg=MUTED,font=(FONT,8,"bold")).pack(anchor="w",padx=14,pady=(13,8));
        self.advanced_text=tk.StringVar(value="Kill switch   Available\nDNS protection  Active after connect\nAuto-connect  Ready\nSplit tunneling  Ready")
        tk.Label(adv,textvariable=self.advanced_text,bg=PANEL,fg=TEXT,justify="left",font=(FONT,9),anchor="nw").pack(fill="both",expand=True,padx=14,pady=(0,12))
    ...
    def _premium_pump(self):
        ...
        if events:
            try:
                while True:
                    kind, _, text = events.get_nowait()
                    if kind=="servers": self._render()
                    elif kind=="status": self._set_busy(True,text)
                    elif kind=="connected":
                        self._connected=True; self._connected_at=time.monotonic(); self._set_busy(False); self.hero_state.configure(text="PROTECTED",fg=SUCCESS); self.power.configure(text="DISCONNECT",bg=SUCCESS,fg=BG); self.status.set(text); self.status_pill.configure(text="● ON",fg=SUCCESS); server=getattr(self,"selected_server",{}) or {}; loc=server.get("country") or "Secure route"; city=server.get("city") or ""; self.hero_location.configure(text=f"{loc}{('  •  '+city) if city else ''}"); ip=getattr(getattr(self,"_session_controller",None),"session",None); public=getattr(ip,"public_ip",None) if ip else None; self.hero_ip.configure(text=f"Exit IP  {public or server.get('ip') or 'Verified'}"); self.route_name.set(f"{loc}{('  •  '+city) if city else ''}"); self.route_ip.set(f"Exit IP  {public or server.get('ip') or 'Verified'}")
                    elif kind=="disconnected": self._connected=False; self._connected_at=None; self._set_busy(False); self.hero_state.configure(text="NOT PROTECTED",fg=MUTED); self.power.configure(text="CONNECT",bg=ACCENT,fg="white"); self.hero_location.configure(text="Automatic"); self.hero_ip.configure(text="Exit IP  —"); self.route_name.set("Automatic route"); self.route_ip.set("Exit IP  —")
                    elif kind=="error": self._connected=False; self._set_busy(False); self.hero_state.configure(text="CONNECTION FAILED",fg=DANGER); self.status_pill.configure(text="● ERROR",fg=DANGER); self.status.set("Connection failed"); messagebox.showerror("Findupto VPN",text)
