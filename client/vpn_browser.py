...
class VPNBrowser(tk.Toplevel):
    ...
    def _load_settings(self):
        try:
            ...
            if data.get("search_engine") == "DuckDuckGo":
                self.search_engine = "https://duckduckgo.com/?q="
            elif data.get("search_engine") == "Bing":
    ...
    def _build_ui(self):
        ...
        self.sidebar = tk.Frame(layout, bg=SURFACE, width=205, highlightthickness=1, highlightbackground=BORDER)
        self.sidebar.pack(side="left", fill="y")
        self.sidebar.pack_propagate(False)
        tk.Label(self.sidebar, text="FINDUPTO", bg=SURFACE, fg=TEXT, font=("Segoe UI", 13, "bold")).pack(anchor="w", padx=18, pady=(18, 2))
        tk.Label(self.sidebar, text="SECURE BROWSER PRO", bg=SURFACE, fg=MUTED, font=("Segoe UI", 7, "bold")).pack(anchor="w", padx=18, pady=(0, 18))
        ...
        self.vpn_status = tk.Label(self.sidebar, text="● VPN PROTECTED\nTraffic routed through Findupto VPN", bg=PANEL_2, fg=SUCCESS, justify="left", font=("Segoe UI", 8, "bold"), padx=12, pady=10)
    ...
    def _side(self, text, command, active=False):
        button = tk.Button(self.sidebar, text=text, command=command, anchor="w", bg=ACCENT if active else SURFACE, fg=TEXT if active else MUTED, activebackground=PANEL_2, activeforeground=TEXT, relief="flat", bd=0, cursor="hand2", font=("Segoe UI", 9, "bold"), padx=14, pady=10)
        button.pack(fill="x", padx=10, pady=2)
        return button

    def _top_button(self, parent, text, command, accent=False):
        return tk.Button(parent, text=text, command=command, bg=ACCENT if accent else PANEL_2, fg=TEXT, activebackground=ACCENT_2, activeforeground=TEXT, relief="flat", bd=0, cursor="hand2", font=("Segoe UI", 9, "bold"), padx=10, pady=6).pack(side="left", padx=2, pady=7)
    ...
    def _show_start_page(self):
        ...
        tk.Label(self.viewer, text=title, bg="#0b0f18", fg=TEXT, font=("Segoe UI", 25, "bold")).pack(pady=(90, 8))
        tk.Label(self.viewer, text="VPN protected • Tracker protection • Secure navigation", bg="#0b0f18", fg=MUTED, font=("Segoe UI", 10)).pack(pady=(0, 24))
        search = ttk.Entry(self.viewer, font=("Segoe UI", 13), width=70)
        search.pack(ipady=9)
        search.focus_set()
        search.bind("<Return>", lambda _e: self.load(search.get()))
        tk.Label(self.viewer, text="Type a URL or search term in the address bar.", bg="#0b0f18", fg="#65748a", font=("Segoe UI", 8)).pack(pady=10)
    ...
    def _render_tabs(self):
        ...
        for index, tab in enumerate(self.tabs):
            ...
            tk.Button(frame, text=title, command=lambda i=index: self.switch_tab(i), bg=PANEL_2 if active else SURFACE, fg=TEXT if active else MUTED, activebackground=PANEL_2, relief="flat", bd=0, font=("Segoe UI", 8, "bold"), padx=10, pady=7).pack(side="left")
            tk.Button(frame, text="×", command=lambda i=index: self.close_tab(i), bg=PANEL_2 if active else SURFACE, fg=MUTED, activebackground=PANEL_2, relief="flat", bd=0, font=("Segoe UI", 8, "bold"), padx=6, pady=7).pack(side="left")
        tk.Button(self.tabbar, text="＋", command=self.new_tab, bg=SURFACE, fg=CYAN, relief="flat", bd=0, font=("Segoe UI", 10, "bold"), padx=9, pady=5).pack(side="left")
    ...
    def forward(self):
        if self.forward_history:
            self.load(self.forward_history.pop(), add_history=True)
        else:
    ...
    def _show_list(self, title, items, empty, action):
        ...
        for item in items:
            ...
            tk.Button(box, text=text, anchor="w", command=lambda x=item: action(x) if action else None, bg=PANEL, fg=TEXT, activebackground=PANEL_2, activeforeground=TEXT, relief="flat", bd=0, font=("Segoe UI", 9), padx=15, pady=10).pack(fill="x", pady=1)
    ...
    def show_privacy(self):
        for child in self.viewer.winfo_children():
            child.destroy()
        tk.Label(self.viewer, text="Privacy Center", bg="#0b0f18", fg=TEXT, font=("Segoe UI", 20, "bold")).pack(anchor="w", padx=25, pady=(25, 18))
        ...
        tk.Label(self.viewer, text="VPN protection is provided by the active Findupto VPN tunnel.", bg="#0b0f18", fg=SUCCESS, font=("Segoe UI", 9, "bold")).pack(anchor="w", padx=25, pady=20)

    def _privacy_toggle(self, label, attr):
        value = tk.BooleanVar(value=getattr(self, attr))
        tk.Checkbutton(self.viewer, text=label, variable=value, command=lambda: self._set_privacy(attr, value.get()), bg="#0b0f18", fg=TEXT, selectcolor=PANEL_2, activebackground="#0b0f18", activeforeground=TEXT, font=("Segoe UI", 10), anchor="w").pack(fill="x", padx=25, pady=5)
    ...
    def show_settings(self):
        for child in self.viewer.winfo_children():
            child.destroy()
        tk.Label(self.viewer, text="Browser Settings", bg="#0b0f18", fg=TEXT, font=("Segoe UI", 20, "bold")).pack(anchor="w", padx=25, pady=(25, 18))
        row = tk.Frame(self.viewer, bg=PANEL)
        row.pack(fill="x", padx=25, pady=5)
        tk.Label(row, text="Default search engine", bg=PANEL, fg=TEXT, font=("Segoe UI", 10, "bold")).pack(side="left", padx=15, pady=14)
        ...
        if "duckduckgo" in self.search_engine:
            ...
            current = "Bing"
        engine.set(current)
        engine.bind("<<ComboboxSelected>>", lambda _e: self.set_search_engine(engine.get()))
        engine.pack(side="right", padx=15, pady=10)
        zoom = tk.Frame(self.viewer, bg=PANEL)
        zoom.pack(fill="x", padx=25, pady=5)
        tk.Label(zoom, text="Page zoom", bg=PANEL, fg=TEXT, font=("Segoe UI", 10, "bold")).pack(side="left", padx=15, pady=14)
    ...
    def set_search_engine(self, engine):
        ...
        }.get(engine, self.search_engine)
        self._save_settings()
        self.status.config(text=f"● Search engine: {engine}")
