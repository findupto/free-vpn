...
class App(PremiumApp):
    ...
    def _configure_styles(self):
        ...
        s.map("Treeview", background=[("selected", "#303e5d")], foreground=[("selected", TEXT)])
        ...
        s.configure("TSpinbox", padding=7, font=(FONT, 9), fieldbackground=PANEL_2,
                    background=PANEL_2, foreground=TEXT, arrowcolor=ACCENT_2)
        s.configure("TCheckbutton", padding=6, font=(FONT, 9, "bold"), background=PANEL, foreground=TEXT)
    ...
    def _button(self, parent, text, command, kind="secondary", compact=False):
        palette = {"primary": (ACCENT, "#ffffff", ACCENT_2), "success": (SUCCESS, BG, "#67efc6"),
                   "danger": (DANGER, BG, "#ff9aac"), "secondary": (PANEL_3, TEXT, "#2a3d5b"),
                   "ghost": (SURFACE, MUTED, PANEL_2), "blue": (BLUE, "#ffffff", "#79b0ff")}
        ...
        b = tk.Button(parent, text=text, command=command, bg=bg, fg=fg,
                      ...
                      highlightthickness=1, highlightbackground=BORDER_HI, highlightcolor=ACCENT_2,
    ...
    def _build(self):
        ...
        self.sidebar = tk.Frame(self.root, bg=SURFACE, width=244, highlightthickness=1, highlightbackground=BORDER)
        self.sidebar.pack(side="left", fill="y"); self.sidebar.pack_propagate(False); self._build_premium_sidebar()
        self.content = tk.Frame(self.root, bg=BG); self.content.pack(side="left", fill="both", expand=True); self._build_premium_content()
        self._apply_responsive_layout(self.winfo_width() or 1280)
        self.after(500, self._render_session)

    def _build_premium_sidebar(self):
        brand = tk.Frame(self.sidebar, bg=SURFACE); brand.pack(fill="x", padx=18, pady=(22, 24))
        logo = tk.Frame(brand, bg=ACCENT, width=46, height=46); logo.pack(side="left", padx=(0, 12)); logo.pack_propagate(False)
        ...
        tk.Label(self.sidebar, text="COMMAND CENTER", bg=SURFACE, fg=MUTED, font=(FONT, 7, "bold")).pack(anchor="w", padx=20, pady=(0, 8))
        self.nav_items = []
        for icon, label, command in (("⌂", "Overview", self._focus_dashboard), ("◈", "Server Network", self._focus_servers), ("⚡", "Fast Routes", self._focus_quick), ("◉", "Diagnostics", self.open_log)):
            b = tk.Button(self.sidebar, text=f"  {icon}   {label}", anchor="w", command=command,
                          bg=ACCENT if label == "Overview" else SURFACE, fg="#ffffff" if label == "Overview" else MUTED,
                          activebackground=PANEL_2, activeforeground=TEXT, relief="flat", bd=0,
                          cursor="hand2", font=(FONT, 10, "bold"), padx=18, pady=12)
            b.pack(fill="x", padx=10, pady=2); self.nav_items.append(b)
        tk.Frame(self.sidebar, bg=SURFACE).pack(fill="both", expand=True)
        status = self._card(self.sidebar, bg=PANEL, accent=True); status.pack(fill="x", padx=14, pady=14)
        ...
        tk.Label(status, textvariable=self.side_status, bg=PANEL, fg=TEXT, font=(FONT, 9, "bold"), wraplength=190, justify="left").pack(anchor="w", padx=13, pady=(0, 12))
        tk.Label(self.sidebar, text=f"v{engine.APP_VERSION}  •  Live route intelligence", bg=SURFACE, fg=MUTED, font=(FONT, 7)).pack(anchor="w", padx=20, pady=(0, 18))

    def _build_premium_content(self):
        self.header = tk.Frame(self.content, bg=BG); self.header.pack(fill="x", padx=30, pady=(22, 13))
        left = tk.Frame(self.header, bg=BG); left.pack(side="left", fill="x", expand=True)
        tk.Label(left, text="SECURE NETWORK  /  COMMAND CENTER", bg=BG, fg=ACCENT_2, font=(FONT, 8, "bold")).pack(anchor="w")
        tk.Label(left, text="Your private network, beautifully controlled.", bg=BG, fg=TEXT, font=(FONT, 25, "bold")).pack(anchor="w", pady=(2, 0))
        tk.Label(left, text="Discover live routes, compare performance and connect with confidence.", bg=BG, fg=MUTED, font=(FONT, 9)).pack(anchor="w", pady=(3, 0))
        self.status_pill = self._pill(self.header, "●  READY", PANEL_2, MUTED); self.status_pill.pack(side="right", pady=(8, 0))
        ...
        tk.Label(hero_left, text="One elegant tap. The best verified route.", bg=SURFACE, fg=TEXT, font=(FONT, 15, "bold")).pack(anchor="w", pady=(4, 2))
        tk.Label(hero_left, text="Latency • availability • throughput • route quality", bg=SURFACE, fg=MUTED, font=(FONT, 8)).pack(anchor="w")
        self.best_btn = self._button(self.hero, "✦  CONNECT SMART", self.best, "primary"); self.best_btn.pack(side="right", padx=22, pady=20)

        self.command = self._card(self.content, bg=PANEL); self.command.pack(fill="x", padx=30, pady=(0, 10))
        ...
        self.session_label = tk.Label(self.command, text="SESSION  00:00:00", bg=PANEL_2, fg=CYAN, padx=13, pady=8, font=(FONT, 8, "bold"), highlightthickness=1, highlightbackground=BORDER_HI)
        self.session_label.pack(side="right", padx=6, pady=7)

        self.metrics = tk.Frame(self.content, bg=BG); self.metrics.pack(fill="x", padx=30, pady=(0, 10)); self.metric_vars = {}
        for key, title, accent in (("visible", "VISIBLE ROUTES", CYAN), ("online", "VERIFIED ONLINE", SUCCESS), ("fast", "FAST ROUTES", ACCENT_2), ("countries", "COUNTRIES", WARNING), ("tested", "TESTED", TEXT)):
            c = self._card(self.metrics, bg=PANEL); c.pack(side="left", fill="x", expand=True, padx=(0, 7))
        ...
        self.speed_status = tk.StringVar(value="Waiting for network scan"); tk.Label(qhead, textvariable=self.speed_status, bg=PANEL, fg=SUCCESS, font=(FONT, 8, "bold")).pack(side="right")
        ...
        cols = ("status", "country", "city", "endpoint", "routes", "latency", "speed", "source"); self._table_columns = cols
        self.tree = ttk.Treeview(frame, columns=cols, show="headings", selectmode="browse")
        for col, heading in zip(cols, ("STATUS", "COUNTRY", "CITY", "ENDPOINT", "IP ROUTES", "LATENCY", "SPEED", "SOURCE")):
            self.tree.heading(col, text=heading); self.tree.column(col, width=110, minwidth=58, stretch=True)
        ...
        self.refresh_btn = self._button(self.action_bar, "↻  SCAN NETWORK", self.refresh, "blue"); self.refresh_btn.pack(side="left")
        self.sel_btn = self._button(self.action_bar, "➜  CONNECT SELECTED", self.selected); self.sel_btn.pack(side="left", padx=7)
        self.diag_btn = self._button(self.action_bar, "◉  OPEN DIAGNOSTICS", self.open_log, "ghost"); self.diag_btn.pack(side="left")
        self.disconnect_btn = self._button(self.action_bar, "■  DISCONNECT", self.disconnect, "danger"); self.disconnect_btn.pack(side="right")
    ...
    def _build_filters_premium(self):
        ...
        ttk.Checkbutton(self.filters, text="Fast", variable=self.fast_only, command=self._render).pack(side="left", padx=3); ttk.Checkbutton(self.filters, text="Online", variable=self.available_only, command=self._render).pack(side="left", padx=3)
        ...
        for label, var in (("COUNTRY", self.country), ("CITY", self.city), ("SOURCE", self.source)):
            ...
            cb = ttk.Combobox(self.filters, textvariable=var, state="readonly", width=11); cb.pack(side="left", padx=(0, 4)); cb.bind("<<ComboboxSelected>>", lambda _e: self._render()); setattr(self, label.lower() + "_combo", cb)
        ...
        tk.Label(self.filters, text="ms", bg=PANEL, fg=MUTED, font=(FONT, 8)).pack(side="left", padx=(3, 8)); ttk.Checkbutton(self.filters, text="Auto Connect", variable=self.auto_connect, command=self._auto_connect_changed).pack(side="left", padx=4)
    ...
    def _render_quick(self, items):
        ...
        for s in items[:count]:
            ping = float(s.get("live_ping", 9999)); name = s.get("city") or s.get("country") or s.get("host") or "Unknown route"; country = str(s.get("country") or "Global"); ips = s.get("ips") or [s.get("ip") or s.get("host", "")]
            c = self._card(self.quick_frame, bg=PANEL_2, accent=True, glow=ping < 100); c.pack(side="left", fill="x", expand=True, padx=4)
            top = tk.Frame(c, bg=PANEL_2); top.pack(fill="x", padx=12, pady=(9, 2)); tk.Label(top, text="● ONLINE", bg=PANEL_2, fg=SUCCESS, font=(FONT, 7, "bold")).pack(side="left"); tk.Label(top, text=f"{ping:.0f} ms", bg=PANEL_2, fg=CYAN, font=(FONT, 8, "bold")).pack(side="right")
            tk.Label(c, text=str(name)[:24], bg=PANEL_2, fg=TEXT, font=(FONT, 11, "bold")).pack(anchor="w", padx=12); tk.Label(c, text=f"{country}  •  {len(ips)} verified IP routes", bg=PANEL_2, fg=MUTED, font=(FONT, 8)).pack(anchor="w", padx=12, pady=(2, 3))
            meter = tk.Frame(c, bg=PANEL_3, height=5); meter.pack(fill="x", padx=12, pady=(4, 4)); ratio = max(0.08, min(1.0, 1.0 - ping / 600.0)); tk.Frame(meter, bg=SUCCESS if ping <= 120 else WARNING, height=5, width=max(10, int(100 * ratio))).pack(side="left")
            tk.Label(c, text=str(ips[0])[:36], bg=PANEL_2, fg=MUTED, font=(FONT, 7)).pack(anchor="w", padx=12); self._button(c, "CONNECT  →", lambda x=s: self._connect([x]), "primary", True).pack(anchor="e", padx=10, pady=(6, 10))
    ...
    def _render(self):
        ...
        self.table_hint.configure(text=f"{len(self.servers):,} catalog endpoints  •  {available:,} verified  •  live intelligence"); self._render_quick(visible); self._update_combos()

    def _discover_worker(self):
        try:
            data = engine.discover(40); unique, seen = [], set()
            for s in data:
                ips = s.get("ips") or [s.get("ip") or s.get("host", "")]; key = tuple(sorted(str(x) for x in ips if x)) or (str(s.get("host", "")),)
    ...
    def disconnect(self):
        super().disconnect(); self._session_started = None; self.session_label.configure(text="SESSION  00:00:00")
