...
class App(tk.Tk):
    """Responsive VPN dashboard that adapts between wide and compact displays."""
    ...
    def _configure_styles(self):
        ...
        s.configure("Treeview", background=PANEL, fieldbackground=PANEL, foreground=TEXT,
                    rowheight=42, borderwidth=0, font=(FONT, 9))
        s.configure("Treeview.Heading", background=PANEL_2, foreground=MUTED,
                    relief="flat", font=(FONT, 8, "bold"), padding=(7, 8))
        s.map("Treeview", background=[("selected", "#28344a")], foreground=[("selected", TEXT)])
    ...
    def _button(self, parent, text, command, kind="secondary", compact=False):
        palette = {
            ...
            "success": (SUCCESS, BG, "#55e7b9"),
            "danger": (DANGER, BG, "#ff8ca0"),
        ...
        return tk.Button(parent, text=text, command=command, bg=bg, fg=fg,
                         ...
                         padx=10 if compact else 14, pady=7 if compact else 9)
    ...
    def _build_sidebar(self):
        ...
        tk.Label(brand, text="F", bg=ACCENT, fg="white", width=3, pady=7,
                 font=(FONT, 14, "bold")).pack(side="left", padx=(0, 10))
        text = tk.Frame(brand, bg=SURFACE)
        text.pack(side="left")
        tk.Label(text, text="FINDUPTO", bg=SURFACE, fg=TEXT,
                 font=(FONT, 13, "bold")).pack(anchor="w")
        tk.Label(text, text="VPN DASHBOARD", bg=SURFACE, fg=MUTED,
        ...
        for icon, label in (("⌂", "Dashboard"), ("◉", "Servers"),
            ...
            b = tk.Button(self.sidebar, text=f"  {icon}   {label}", anchor="w",
                          ...
                          bg=ACCENT if label == "Dashboard" else SURFACE,
        ...
        info = self._card(self.sidebar)
        info.pack(fill="x", padx=14, pady=14)
        tk.Label(info, text="CONNECTION", bg=PANEL, fg=MUTED,
                 font=(FONT, 7, "bold")).pack(anchor="w", padx=12, pady=(10, 2))
        self.side_status = tk.StringVar(value="Ready to connect")
        tk.Label(info, textvariable=self.side_status, bg=PANEL, fg=SUCCESS,
                 font=(FONT, 9, "bold"), wraplength=170, justify="left").pack(anchor="w", padx=12, pady=(0, 11))
        tk.Label(self.sidebar, text=f"v{VERSION}  •  Secure tunnel", bg=SURFACE,
    ...
    def _build_content(self):
        ...
        tk.Label(left, text="VPN Control Center", bg=BG, fg=TEXT,
                 font=(FONT, 23, "bold")).pack(anchor="w")
        tk.Label(left, text="Find the fastest verified endpoint and connect in one click.",
                 bg=BG, fg=MUTED, font=(FONT, 9)).pack(anchor="w", pady=(2, 0))
        self.status = tk.StringVar(value="Preparing instant-connect pool…")
        self.status_pill = tk.Label(self.header, textvariable=self.status, bg=PANEL,
                                    fg=MUTED, padx=13, pady=8, font=(FONT, 8, "bold"),
                                    highlightthickness=1, highlightbackground=BORDER)
        self.status_pill.pack(side="right", padx=(10, 0))

        self.filters = self._card(self.content)
        self.filters.pack(fill="x", padx=26, pady=(0, 10))
        self._build_filters()

        self.stats = tk.Frame(self.content, bg=BG)
        self.stats.pack(fill="x", padx=26, pady=(0, 10))
        self.stat_cards = {}
        for key, label in (("shown", "SHOWN"), ("available", "AVAILABLE"),
                           ("fast", "FAST"), ("pool", "QUICK POOL"), ("tested", "TESTED")):
            c = self._card(self.stats)
            c.pack(side="left", fill="x", expand=True, padx=(0, 7))
            value = tk.StringVar(value="0")
            tk.Label(c, textvariable=value, bg=PANEL, fg=TEXT,
                     font=(FONT, 18, "bold")).pack(anchor="w", padx=13, pady=(8, 0))
            tk.Label(c, text=label, bg=PANEL, fg=MUTED,
                     font=(FONT, 7, "bold")).pack(anchor="w", padx=13, pady=(0, 9))
            self.stat_cards[key] = value

        self.quick = self._card(self.content)
        self.quick.pack(fill="x", padx=26, pady=(0, 10))
        qhead = tk.Frame(self.quick, bg=PANEL)
        qhead.pack(fill="x", padx=14, pady=(10, 5))
        tk.Label(qhead, text="⚡  FASTEST VERIFIED", bg=PANEL, fg=TEXT,
        ...
        self.server_card.pack(fill="both", expand=True, padx=26, pady=(0, 10))
        top = tk.Frame(self.server_card, bg=PANEL)
        top.pack(fill="x", padx=14, pady=(10, 7))
        tk.Label(top, text="Server Lounge", bg=PANEL, fg=TEXT,
                 font=(FONT, 12, "bold")).pack(side="left")
        self.table_hint = tk.Label(top, text="Select a verified server to connect",
                                   bg=PANEL, fg=MUTED, font=(FONT, 8))
    ...
    def _build_filters(self):
        tk.Label(self.filters, text="FILTERS", bg=PANEL, fg=MUTED,
                 font=(FONT, 7, "bold")).pack(side="left", padx=(14, 7), pady=11)
        ...
        ttk.Checkbutton(self.filters, text="Fast", variable=self.fast_only,
                        command=self._render).pack(side="left", padx=4)
        ttk.Checkbutton(self.filters, text="Available", variable=self.available_only,
                        command=self._render).pack(side="left", padx=4)
        ...
        for label, var in (("COUNTRY", self.country), ("CITY", self.city), ("SOURCE", self.source)):
            tk.Label(self.filters, text=label, bg=PANEL, fg=MUTED,
                     font=(FONT, 7, "bold")).pack(side="left", padx=(10, 4))
        ...
        tk.Label(self.filters, text="MAX PING", bg=PANEL, fg=MUTED,
                 font=(FONT, 7, "bold")).pack(side="left", padx=(10, 4))
        self.max_ping = tk.IntVar(value=250)
        self.ping_spin = ttk.Spinbox(self.filters, from_=50, to=2000, increment=25,
                                     width=6, textvariable=self.max_ping, command=self._render)
        self.ping_spin.pack(side="left")
        tk.Label(self.filters, text="ms", bg=PANEL, fg=MUTED,
                 font=(FONT, 8)).pack(side="left", padx=(3, 8))
    ...
    def _apply_responsive_layout(self, width):
        ...
        if narrow:
            ...
            self.header.pack_configure(padx=26, pady=(20, 12))
    ...
    def _set_table_mode(self, mode):
        widths = {
            "wide": (92, 100, 105, 180, 210, 72, 90, 95),
            "compact": (80, 90, 90, 145, 170, 68, 82, 85),
            "narrow": (72, 82, 82, 135, 150, 65, 78, 80),
        }[mode]
    ...
    def _probe(server):
        ...
        return dict(server, available=True, live_ping=best, ping=best, ip=best_host,
                    host=server.get("host") or best_host, ips=[e.host for e in eps],
                    rank=float(server.get("rank", 0)) + max(0, 500 - best))
    ...
    def _eligible(self):
        try:
            ...
            limit = FAST_LIMIT_MS
        items = [s for s in self.servers
                 if (not self.available_only.get() or s.get("available"))
                 and (not self.fast_only.get() or float(s.get("live_ping", 9999)) <= limit)]
    ...
    def _update_combos(self):
        for key, combo in (("country", self.country_combo), ("city", self.city_combo),
    ...
    def _render_quick(self, items):
        for w in self.quick_frame.winfo_children():
            w.destroy()
        if not items:
            tk.Label(self.quick_frame, text="No servers match the current filters.",
                     bg=PANEL, fg=MUTED, font=(FONT, 9)).pack(anchor="w", padx=8, pady=12)
            return
        ...
        for s in items[:count]:
            ...
            card = tk.Frame(self.quick_frame, bg=PANEL_2, highlightthickness=1,
                            highlightbackground=BORDER_HI)
            card.pack(side="left", fill="x", expand=True, padx=3)
            tk.Label(card, text=f"●  {name}", bg=PANEL_2, fg=SUCCESS,
                     font=(FONT, 8, "bold")).pack(anchor="w", padx=9, pady=(7, 0))
            tk.Label(card, text=f"{ping:.0f} ms  •  {len(ips)} IPs", bg=PANEL_2, fg=TEXT,
                     font=(FONT, 9)).pack(anchor="w", padx=9)
            if not self.compact:
                tk.Label(card, text="  ".join(str(x) for x in ips[:2]), bg=PANEL_2,
                         fg=MUTED, font=(FONT, 7)).pack(anchor="w", padx=9, pady=(0, 4))
            tk.Button(card, text="CONNECT", command=lambda x=s: self._connect([x]),
                      bg=ACCENT, fg="white", activebackground=ACCENT_2,
                      relief="flat", bd=0, cursor="hand2", font=(FONT, 7, "bold"),
                      padx=8, pady=4).pack(anchor="e", padx=8, pady=(0, 7))
    ...
    def _render(self):
        ...
        for s in visible:
            ...
            self.tree.insert("", "end", iid=str(index[id(s)]), values=(
                "● FAST" if s.get("available") and ping <= FAST_LIMIT_MS else "● ONLINE",
                s.get("country", ""), s.get("city", ""), s.get("host", "") or s.get("ip", ""),
                ", ".join(str(x) for x in ips[:4]), "—" if ping >= 9999 else f"{ping:.0f} ms",
                f"{float(s.get('speed', 0) or 0):.1f} Mbps" if s.get("speed") else "—",
                s.get("source", ""),
            ))
        available = sum(bool(s.get("available")) for s in self.servers)
        fast = sum(bool(s.get("available")) and float(s.get("live_ping", 9999)) <= FAST_LIMIT_MS
        ...
        for server in candidates:
            ...
            try:
                self.events.put(("status", None,
                                 f"✦ {server.get('host') or server.get('ip')} • {server.get('live_ping', 0):.0f} ms • connecting…"))
                ...
                self.events.put(("connected", None,
                                 f"CONNECTED • {server.get('host') or server.get('ip')} • {server.get('live_ping', 0):.0f} ms • IP {ip}"))
                return
            except Exception as exc:
