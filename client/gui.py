...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        frame = ttk.Frame(self, padding=(14,0,14,0)); frame.pack(fill="both", expand=True)
        cols=("country","city","host","ping","speed","source")
        self.tree=ttk.Treeview(frame,columns=cols,show="headings")
        for col,width in zip(cols,(140,150,260,90,110,120)):
        ...
        self.refresh_btn=ttk.Button(bar,text="Refresh",command=self.refresh); self.refresh_btn.pack(side="left")
        self.best_btn=ttk.Button(bar,text="Connect Best",command=self.best); self.best_btn.pack(side="left",padx=7)
    ...
    def refresh(self):
        if self.busy: return
        self._set_busy(True); self.status.set("Discovering live VPN sources (hard 8s deadline)..."); engine.log("UI REFRESH")
        threading.Thread(target=self._discover_worker,daemon=True).start()

    def _discover_worker(self):
        try:
            data=engine.discover(8.0)
            self.events.put(("servers",data,f"{len(data)} candidates ready; ranked by provider metrics"))
    ...
    def _connect(self,candidates):
        if self.busy: return
        self._set_busy(True); self.status.set(f"Testing {len(candidates)} candidates; success requires initialization + public-IP verification...")
    ...
    def _connect_worker(self,candidates):
        ...
        for server in candidates:
            ...
            try:
                ...
                self.events.put(("connected",None,f"CONNECTED — {server['host']} — public IP {ip}")); return
            except Exception as exc:
