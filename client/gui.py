...
APP='Findupto VPN';VERSION='9.2.0'
class App(tk.Tk):
 def __init__(self):
  super().__init__();self.title(f'{APP} {VERSION}');self.geometry('1180x720');self.minsize(920,580);self.events=queue.Queue();self.servers=[];self.process=None;self.tmp=None;self.current_log=None;self.busy=False;self._build();self.after(100,self._pump);self.refresh()
 def _build(self):
  ...
  for col,width in zip(cols,(140,150,280,90,110,120)):self.tree.heading(col,text=col.title());self.tree.column(col,width=width,anchor='w')
 ...
 def _discover_worker(self):
  try:self.events.put(('servers',engine.discover(12), 'Live catalog ready; tunnel verification is mandatory'))
 ...
 def best(self):
  ...
  # Prefer live VPN Gate candidates; keep a few VPNBook fallbacks only when needed.
  gate=[s for s in self.servers if s.get('kind')=='gate'][:12];book=[s for s in self.servers if s.get('kind')=='book'];self._connect(gate+book)
 def selected(self):
 ...
 def _connect(self,candidates):
  if self.busy:return
  self._set_busy(True);self.status.set(f'Testing {len(candidates)} candidates; CONNECTED is shown only after route + public-IP verification...');threading.Thread(target=self._connect_worker,args=(candidates,),daemon=True).start()
 def _connect_worker(self,candidates):
  ...
  for server in candidates:
   ...
   try:
    ...
    self.process,self.tmp,self.current_log=process,tmp,logfile;engine.log(f'VPN CONNECTED AND VERIFIED server={server["host"]} public_ip={ip}');self.events.put(('connected',None,f"CONNECTED — {server['host']} — public IP {ip}"));return
