...
def elevate():
    ...
    try:
        ...
        return ctypes.windll.shell32.ShellExecuteW(
            None, "runas", exe, params, os.path.dirname(exe), 1
        ) > 32
...
def _powershell(url, dst, timeout=7):
    ...
    script = (
        ...
        f"Invoke-WebRequest -UseBasicParsing -Uri '{u}' -OutFile '{d}' -TimeoutSec {int(timeout)}"
    )
    r = _run_capture(
...
def _urllib(url, dst, timeout=7):
    ...
    if not size:
        raise RuntimeError("urllib returned an empty file")
    return size


def download(url, dst, software=False):
    """Fail-forward downloader. A 404 ends retries for that URL."""
    errors = []
    for method in (_curl, _powershell, _urllib):
        try:
            ...
            text = str(e)
            errors.append(f"{method.__name__}: {text}")
            log(f"{method.__name__}: {text}")
            low = text.lower()
            if "404" in low or "not found" in low or "410" in low:
                break
...
def ensure_openvpn():
    ...
    local = [
        os.path.join(base, "openvpn-amd64.msi"),
        os.path.join(base, "installer", "openvpn-amd64.msi"),
        os.path.join(os.path.dirname(base), "installer", "openvpn-amd64.msi"),
    ]
        ...
        if not installer:
            ...
            for url in OPENVPN_URLS:
                try:
                    ...
                    log(f"OpenVPN installer source failed: {url}: {e}")
            else:
                raise RuntimeError("OpenVPN installer could not be downloaded.")
        msi_log = os.path.join(td, "openvpn-msi.log")
        r = subprocess.run(
            ["msiexec.exe", "/i", installer, "/qn", "/norestart", "/L*v", msi_log],
...
def parse_servers(text):
    ...
    for raw in lines[header_i + 1:]:
        ...
        servers.append({
            "ip": ip,
            "country": (d.get("CountryLong") or "Unknown").strip(),
            "ping_ms": ping,
            "speed_mbps": speed,
            "score": score,
            "config_b64": b64,
    ...
    servers.sort(key=lambda s: (
        s["ping_ms"] if s["ping_ms"] is not None else 99999,
        -s["speed_mbps"], -s["score"]
    ))
    return servers[:80]
...
def fetch_one_source(url, result_q):
    try:
        servers = parse_servers(get_bytes(url).decode("utf-8", errors="replace"))
        if servers:
            result_q.put(("ok", url, servers))
        else:
            result_q.put(("bad", url, "empty"))
    except Exception as e:
        result_q.put(("bad", url, str(e)))


def fetch_servers():
    # Deliberately not an executor context manager: its __exit__ waits for
    # slow requests after a good source has already returned.
    result_q = queue.Queue()
    for url in VPN_GATE_SOURCES:
        threading.Thread(target=fetch_one_source, args=(url, result_q), daemon=True).start()

    deadline = time.monotonic() + 9
    failures = []
    while time.monotonic() < deadline:
        try:
            kind, url, value = result_q.get(timeout=.25)
        except queue.Empty:
            continue
        if kind == "ok":
            save_cache(value)
            log(f"server source selected: {url} ({len(value)} servers)")
            return value
...
def profile_variants(server):
    ...
    for line in lines:
        ...
        if len(p) >= 3 and p[0].lower() == "remote":
            try:
                ...
                continue
            proto = "udp" if "udp" in p[1].lower() else "tcp"
            remotes.append((proto, port))
    ...
    def build(mode=None):
        ...
        for line in lines:
            p = line.strip().split()
            if p and p[0].lower() == "auth-user-pass":
                continue
            if len(p) >= 3 and p[0].lower() == "remote":
                had_remote = True
                if mode:
                    ...
                    proto = "udp" if p[1].lower().startswith("udp") else "tcp"
                    port = int(p[2])
                out.append(f"remote {server['ip']} {port}")
                if mode:
                    out.append(f"proto {'udp' if proto == 'udp' else 'tcp-client'}")
            elif p and p[0].lower() == "proto" and mode:
                continue
            else:
                out.append(line)
        if not had_remote:
...
def openvpn_try(ovpn, config, server, auth_path, variant):
    ...
    stdout_path = os.path.join(td, "openvpn-stdout.log")
    full = add_auth(config, auth_path)
    with open(config_path, "w", encoding="utf-8", newline="\n") as f:
        f.write(full)

    cmd = [
        ovpn, "--config", config_path, "--log", log_path,
        "--writepid", os.path.join(td, "openvpn.pid"),
    ]
        ...
        for path in (stdout_path, log_path):
            try:
                with open(path, encoding="utf-8", errors="replace") as f:
                    output.append(f.read()[-3500:])
            except OSError:
                pass
        detail = "\n".join(x for x in output if x).strip()
        if not detail:
            detail = f"OpenVPN exited with code {proc.returncode} without diagnostics."
...
class App(tk.Tk):
    ...
    def startup(self):
        ...
        if cached:
            ...
            self.status.set(f"{len(cached)} cached servers ready • updating in background…")
    ...
    def refresh(self, background=False):
        ...
        self.loading = True
        if not background:
            self.status.set("Refreshing in background…")
    ...
    def _refresh_worker(self):
        try:
            ...
            log(f"refresh: {e}")
            if not self.servers:
                self.events.put(("status", "Network unavailable — cached list will be used when available"))
        finally:
            self.loading = False
    ...
    def show_servers(self, servers):
        ...
        for i, s in enumerate(self.servers[:60]):
            self.tree.insert("", "end", iid=str(i), values=(
                s.get("country", "Unknown"), s.get("ip", ""),
                "-" if s.get("ping_ms") is None else f"{s['ping_ms']:.0f} ms",
                f"{s.get('speed_mbps', 0):.1f} Mbps", s.get("source", "VPN Gate")
            ))
        if self.servers:
    ...
    def fast_connect(self):
        ...
        self.status.set("Fast Connect: testing best servers…")
        threading.Thread(target=self.auto_connect, args=(self.servers[:8],), daemon=True).start()
    ...
    def auto_connect(self, candidates):
        ...
        try:
            ...
            try:
                ...
                self.events.put(("error", f"OpenVPN is unavailable: {e}"))
                return
                    ...
                    for name, cfg in profile_variants(s):
                        for ip, port in config_endpoints(cfg):
                            if tcp_reachable(ip, port):
                                with lock:
                                    live.append((s, name, cfg))
                                return
                except Exception as e:
                    log(f"probe {s.get('ip')}: {e}")
            ...
            live_by_ip = {x[0]["ip"] for x in live}
            ordered = []
            for s in candidates:
                for item in live:
                    if item[0]["ip"] == s["ip"]:
            ...
            self.events.put(("status", "No OpenVPN relay connected"))
            self.events.put(("error", "No VPN tunnel could be established. Public relay servers can reject or time out; the app tested multiple servers and protocol variants. See Diagnostics/log for the exact OpenVPN errors."))
        finally:
            try:
                os.remove(auth)
            except OSError:
    ...
    def diagnostics(self):
        messagebox.showinfo(
            ...
            f"curl: {which('curl.exe') or which('curl') or 'NOT FOUND'}\n"
