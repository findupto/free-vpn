...
def _curl(url, dst, timeout=7):
    ...
    cmd = [exe, "--silent", "--show-error", "--fail", "--location", "--connect-timeout", "3", "--max-time", str(timeout), "-A", UA, "-o", dst, url]
    r = _run_capture(cmd, timeout + 2)
    if r.returncode:
        raise RuntimeError((r.stderr or r.stdout or "curl failed").strip())
    if not os.path.isfile(dst) or not os.path.getsize(dst):
...
def download(url, dst, software=False):
    ...
    for method in (_curl, _powershell, _urllib):
        try:
            ...
            if any(x in text.lower() for x in ("404", "not found", "410")):
                break
...
def fetch_one_source(url, result_q):
    try:
        ...
        result_q.put(("ok", url, servers)) if servers else result_q.put(("bad", url, "empty"))
    except Exception as e:
...
def profile_variants(server):
    ...
    for line in lines:
        p = line.strip().split()
        if p and p[0].lower() == "proto" and len(p) >= 2:
            default_proto = "udp" if p[1].lower().startswith("udp") else "tcp-client"
        ...
        for line in lines:
            ...
            if len(p) >= 3 and p[0].lower() == "remote":
                ...
                if mode:
                    ...
                    explicit = p[3].lower() if len(p) >= 4 else default_proto
                    proto = "udp" if explicit.startswith("udp") else "tcp"
                    port = int(p[2])
...
def openvpn_try(ovpn, config, server, auth_path, variant):
    ...
    with open(config_path, "w", encoding="utf-8", newline="\n") as f:
        f.write(add_auth(config, auth_path))
    cmd = [ovpn, "--config", config_path, "--log", log_path, "--writepid", os.path.join(td, "openvpn.pid")]
    stdout_file = open(stdout_path, "w", encoding="utf-8", errors="replace")
    proc = subprocess.Popen(cmd, stdout=stdout_file, stderr=subprocess.STDOUT, creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0))
    ...
    try:
        ...
        for path in (stdout_path, log_path):
            try:
                ...
                pass
        detail = "\n".join(x for x in output if x).strip() or f"OpenVPN exited with code {proc.returncode} without diagnostics."
...
class App(tk.Tk):
    ...
    def auto_connect(self, candidates):
        ...
        try:
            ...
            ordered = []
            for s in candidates:
                ordered.extend(item for item in live if item[0]["ip"] == s["ip"])
            if not ordered:
            ...
            self.events.put(("error", "No VPN tunnel could be established. Multiple public relays and OpenVPN protocol variants were tested. See Diagnostics/log for the exact errors."))
        finally:
            try:
                os.remove(auth)
            except OSError:
