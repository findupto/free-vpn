...
def _start_elevated() -> bool:
    ...
    script = str(Path(__file__).resolve()); executable = sys.executable
    result = ctypes.windll.shell32.ShellExecuteW(None, "runas", executable, subprocess.list2cmdline([script]), str(Path(script).parent), 1)
    if result <= 32:
        try: ctypes.windll.user32.MessageBoxW(None, "Administrator permission is required for the VPN tunnel.\n\nThe UAC request was cancelled or Windows could not start the elevated application.", "Findupto VPN", 0x10)
...
def _install_gui_compatibility() -> None:
    ...
    if not getattr(LegacyApp, "_findupto_runtime_wired", False):
        ...
        def refresh_with_status(self):
            if not hasattr(self, "status"): self.status = tk.StringVar(master=self, value="Preparing network scan…")
            if not hasattr(self, "side_status"): self.side_status = tk.StringVar(master=self, value="Ready to connect")
        ...
        def _controller(self):
            ...
            if controller is None: controller = SessionController(); self._session_controller = controller
            return controller

        def _connect_verified(self, server, previous_ip=None):
            process, work, logfile = engine.connect(server, total_deadline=75)
            try: vpn_ip = engine.verify_tunnel(previous_ip=previous_ip, timeout=10)
            except Exception: _controller(self).terminate_process(process); raise
        ...
        def start_connection(self, server):
            ...
            if not controller.begin_connect(server):
                self.events.put(("error", None, "A VPN operation is already in progress. Disconnect or wait for it to finish.")); return
            def worker():
                previous_ip = None
                try:
                    self.events.put(("status", None, f"Connecting to {server.get('country', 'VPN')} • verifying the real tunnel…"))
                    try: previous_ip = engine.public_ip(timeout=5)
                    except Exception as exc: engine.log(f"PRECONNECT IP CHECK FAILED error={type(exc).__name__}: {exc}")
                    ...
                    controller.mark_error(); engine.log(f"GUI VPN CONNECTION FAILED host={server.get('host')} error={type(exc).__name__}: {exc}"); self.events.put(("error", None, f"VPN connection failed for {server.get('host', 'selected server')}: {exc}"))
            threading.Thread(target=worker, daemon=True, name="findupto-vpn-connect").start()

        def disconnect(self):
            controller = _controller(self); controller.disconnect(getattr(self, "process", None)); self.process = None; self.current_log = None; self.tmp = None; self.selected_server = None; engine.log("GUI VPN DISCONNECTED"); self.events.put(("disconnected", None, "DISCONNECTED • VPN tunnel closed"))
        ...
        def change_ip(self):
            ...
            if not candidates: controller.mark_error(); self.events.put(("error", None, "No alternate verified VPN server is available. Refresh the network and try again.")); return
            ...
            def worker():
                ...
                for server in candidates[:8]:
                    ...
                    try:
                        self.events.put(("status", None, f"Changing IP • trying {server.get('country', 'VPN')}…")); process, work, logfile, vpn_ip = _connect_verified(self, server, previous_ip)
                        if previous_ip and vpn_ip == previous_ip: controller.terminate_process(process); last_error = RuntimeError("endpoint returned the existing exit IP"); continue
                        self.process = process; self.tmp = work; self.current_log = logfile; self.selected_server = server; controller.mark_connected(process, vpn_ip); engine.log(f"GUI VPN IP CHANGED host={server.get('host')} old_ip={previous_ip} new_ip={vpn_ip}"); self.events.put(("connected", None, f"IP CHANGED • {server.get('country', 'VPN')} • new exit IP {vpn_ip}")); return
                    except Exception as exc: last_error = exc; controller.mark_error(); engine.log(f"GUI VPN IP CHANGE FAILED host={server.get('host')} error={type(exc).__name__}: {exc}")
                self.process = None; self.selected_server = None; message = "Could not obtain a different exit IP. The previous tunnel was closed."; self.events.put(("error", None, message + (f" Last error: {last_error}" if last_error else "")))
            threading.Thread(target=worker, daemon=True, name="findupto-vpn-change-ip").start()

        def open_browser(self):
            process = getattr(self, "process", None)
            if process is None or process.poll() is not None: self.events.put(("error", None, "Connect the VPN first. The Secure Browser is locked until a verified VPN tunnel is active.")); return
