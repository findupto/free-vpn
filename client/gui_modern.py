...
class App(EliteApp):
    ...
    def _build_premium_content(self):
        ...
        # Tk widget options accept a single screen-distance value. A tuple here
        # becomes the Tcl string "2 9" and causes: TclError: expected screen distance.
        ...
        for col, heading in zip(cols, headings): self.tree.heading(col, text=heading); self.tree.column(col, width=105, minwidth=55, stretch=True)
    ...
    def _connect(self, candidates):
        ...
        if candidates: self._set_connection_view(False, candidates[0])
    ...
    def _set_busy(self, value):
        ...
        try: self.status_pill.configure(text="●  SCANNING" if value else "●  READY", fg=WARNING if value else MUTED)
        except tk.TclError: pass
