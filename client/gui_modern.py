...
class App(EliteApp):
    ...
    def _build_premium_content(self):
        ...
        self.header.pack(fill="x", padx=30, pady=(22, 14))
        ...
        ipbox = tk.Frame(top, bg=PANEL_2, highlightthickness=1, highlightbackground=BORDER_HI)
        ipbox.pack(side="right", padx=(14, 0))
        ...
        tk.Label(ipbox, textvariable=self.current_ip, bg=PANEL_2, fg=CYAN, font=(FONT, 14, "bold"), padx=14, pady=(2, 9)).pack(anchor="w")

        controls = tk.Frame(self.hero, bg=SURFACE); controls.pack(fill="x", padx=22, pady=(8, 20))
        self.connect_btn = self._button(controls, "●  CONNECT FASTEST", self.best, "primary")
        ...
        self.browser_btn = self._button(controls, "🌐  SECURE BROWSER", self._open_browser, "secondary")
        self.browser_btn.pack(side="right")
        self.session_label = tk.Label(controls, text="SESSION  00:00:00", bg=PANEL_2, fg=CYAN, padx=12, pady=8, font=(FONT, 8, "bold"), highlightthickness=1, highlightbackground=BORDER_HI)
    ...
    def _set_connection_view(self, connected=False, server=None, ip=None):
        try:
            if connected and server:
                ...
                self.side_status.set(f"Connected via {country} • exit IP {ip or '—'}")
            else:
    ...
    def disconnect(self):
        super().disconnect()
        self._set_connection_view(False)
