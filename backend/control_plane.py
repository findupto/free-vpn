...
class ControlPlane:
    ...
    def heartbeat(self, node_id: str, health_score: int | None = None, latency_ms: float | None = None) -> bool:
        ...
        if latency_ms is not None:
            node.latency_ms = max(0, float(latency_ms))
        return True
    ...
    def get_available_nodes(self, min_health: int = 0) -> list[VPNNode]:
        ...
        return sorted(
            (node for node in self.nodes.values() if node.status == "online" and node.health_score >= threshold),
            key=lambda node: (node.latency_ms, -node.health_score, node.active_sessions),
        )
        ...
        return {
            node.node_id: {
                **asdict(node),
                "host": node.endpoint,
                "ip": node.exit_ip,
                "location": f"{node.city}, {node.country}".strip(", "),
            }
